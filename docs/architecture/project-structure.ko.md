...
```text
src/
├── app/             화면 조립, StudioSession, 자동 저장, 렌더 진단
...
`domain/characterCatalog.ts`는 이미지 캐릭터 7종의 2D/3D 구성과 부분별 입자 수·밝기를 소유한다. `character-art.json`은 배경과 포스터 글자를 제외한 팔레트 마스크이며 `characters.ts`가 외곽·면·색상 경계·볼륨을 결정적으로 샘플링하고 같은 데이터로 아이콘을 만든다. 3D는 실루엣까지의 거리를 이용해 앞뒤 곡면과 내부 깊이를 만든다. 각 부분은 기존 `pattern` ID로 식별하는 독립 레이어이며 저장 필드나 모델 버전은 추가하지 않는다. 색상별 발광 프로필은 레이어 컴파일 때 캐시하고, 입자와 프레임을 React 상태에 넣지 않는다. `scripts/prepare-character-art.py`는 원본 2개를 입력받아 마스크를 재생성하는 개발용 도구다. 기존 9종의 입자 수 제한은 유지하고 캐릭터 내부 면만 최대 4,096점까지 허용한다.
...
`domain/characterShow.ts`는 기존 캐릭터 프리셋을 사용해 2D 7종 다음 3D 7종을 중앙 발사대에서 순차 발사하는 공연을 만든다. 각 큐에 독립 설계를 복사하고 `designDuration`에 잔광 이후 휴지 시간을 더해 다음 발사 tick을 정하므로 캐릭터끼리 겹치지 않는다. 생성한 문서는 저장 스키마로 검증하며 `examples/character-parade.fireworks.json`은 같은 생성기의 저장 결과다. 두 공연의 불러오기 버튼은 `features/show/loadShowTemplate.ts`에서 재생 정지, 문서 교체, 공연 화면 전환, 선택 해제, 시작 시각 복귀를 공유한다. 문서 교체는 기존 이력 명령을 사용하므로 실행 취소할 수 있다.
...
v1 자동 저장 키는 유지하고 v2는 별도 키에 기록한다. 유효하지 않은 v2 자동 저장은 복구 키로 백업한다. 가져오기 실패 시 현재 문서는 바뀌지 않는다. 파일 상한 5MB, 최대 불꽃 64개/레이어 8개/발사대 32개/큐 256개/발사 시작 120초다. 렌더 버퍼는 131,072점, 좌표/색/크기/밝기 합계 4MiB다. 상한에 닿으면 후속 표시를 생략하며 프레임 내 추가 샘플 순회도 중단한다. 캐시/GPU 메모리와 문서 이력은 별도이며 이 제한은 FPS 보장이 아니다.
